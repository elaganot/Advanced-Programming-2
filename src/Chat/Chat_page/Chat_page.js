...
function Chat_page({ contacts, setContacts, username, user_image }) {
    ...
    <div className="chatPage-background-container d-flex" style={{justifyContent: "center"}}>
      <table className='chat-page d-flex' style={{ minWidth: '800px', padding: "2%", marginTop: "5%"}} >
        <th className='left-side' >
          <Left_side contacts={contacts} setContacts={setContacts} user_info={<User_info name={username} image={user_image} />} setChatBox={setChatBox} />
        </th>
        <th className='right-side'>
          {chatBox && <Right_side contact_info={<Contact_info chat={chatBox} />} messages={chatBox.messages} setMessages={chatBox.setMessages} />}
        </th>
      </table>
      </div>
  )
